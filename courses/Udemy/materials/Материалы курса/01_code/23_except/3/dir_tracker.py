...
def snapshot(directory, interactive=True):
    ...
    for root, _, files in os.walk(directory):
        for name in files:
            ...
            try:
                ...
                if interactive:
                    msg = "Если вы продолжите, файл будет помечен как удаленный."
                    msg2 = "\nВы можете попробовать выполнить программу с root правами."
                    print(msg + msg2)
                    ans = input("Продолжить? [Y/n] ").lower().strip()
                    if ans and ans not in {"y", "yes"}:
                        sys.exit()  # bad practice

            except FileNotFoundError:
                print(f"Skipping {path}")
                print("Maybe was deleted/renamed right now...")

            except OSError:
                print(f"Ошибка ОС для {path}")
                if interactive:
                    msg = "Если вы продолжите, файл будет помечен как удаленный."
                    print(msg)
                    ans = input("Продолжить? [Y/n] ").lower().strip()
