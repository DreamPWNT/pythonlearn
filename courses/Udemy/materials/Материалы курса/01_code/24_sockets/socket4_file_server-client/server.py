...
# двойной \r\n\r\n - разделитель заголовков, от самого тела (body) запроса (то что передается).
...
CONNECTIONS = 1     # За раз активных клиентов.
CLIENT_TIMEOUT = 3
...
def send_response(
    ...
    # какой-то директории может быть большим "списком".
    # Давайте для красоты тут циклом напишем. Хотя не обязательно.
...
def handle_request(conn: socket.socket, headers: dict, temp_path: Optional[str] = None):
    ...
    if content_type == "file":
        ...
        shutil.move(temp_path, file)  # Перемещает и переименовывает.
...
def get_request(conn: socket.socket):
    ...
    while b"\r\n\r\n" not in buffer:  # пока не встретим \r\n\r\n
        ...
        if not chunk:
            return {}, False, None  # Пустые чтоб удобно было проверку сделать.
        buffer += chunk
    ...
    fix_path_header(headers)  # В фунции есть доки.
    if not headers:
        send_response(conn, b"Bad request", status=400)
        return {}, False, None
    ...
    if content_type == "text":
        return headers, keep_alive, None  # Если text, то нет смысла читать body. Его просто не будет.
    ...
    temp_path = os.path.join(STORAGE, "temp.tmp")  # Временный файл temp.tmp (расширение пока не важно).
...
def handle_connection(conn: socket.socket, addr: tuple):
    """В цикле постоянно общаемся с клиентов по типу:
    1) Читаем запрос get_request;
    2) Обрабатываем запрос и отвечаем handle_request;
    3) Повторить.
    """
    try:
        keep_alive = True
        ...
        print(f"{e}, client - {addr}") # Кстати addr можно достать из conn.getpeername()
...
def get_connections(sock: socket.socket):
    """Цикл подключения клентов. Пока только 1 за раз."""
    while True:
