...
def fix_path_header(headers: dict[str, str]) -> None:
    """Функция приводит в понятный для сервера путь от клиента.
    Изменяя значение в ключе Path словаря заголовков.
    ...
    if ":" in user_path:
        user_path = user_path[user_path.index(":") + 1 :]  # В пути может быть только 1 : после метки тома.

    # dict - изменяемый объект, эти изменения, будут актуальны вне этой функции.
    headers["Path"] = user_path


def list_directory(path: str) -> bytes | None:
    ...
    try:
        ...
        print(f"У клиента ошибка {e} при просмотре директории:\n{path}")
        return None  # Если None то будем слать статус 404.


def send_file(conn: socket.socket, file_path: str) -> None:
    # Вдруг файла не стало только что, на всякий случай.
    if not os.path.exists(file_path):
        send_response(conn, status=404)  # Без body, просто статуса в заголовках достаточно.
        return
...
def send_response(
    conn: socket.socket, data: bytes = b"", status=200, data_type="text", keep_alive=1
) -> None:
...
def handle_request(conn: socket.socket, headers: dict, temp_path: Optional[str] = None) -> None:
    content_type = headers.get("Content-Type", "text")  # Было бы неплохо сделать
                                                        # Content-Type обязательным.
    ...
    if content_type == "file":
        ...
        send_response(conn)  # Без body, просто статуса 200 в заголовках достаточно.
        return

    # Если клиент отправляет text, а не file, мы это видим по headers.
    path = headers.get("Path", None)
    if not path:
        send_response(conn, status=400)  # Без body, просто статуса в заголовках достаточно.
        return
    else:
    ...
    if os.path.isdir(path):
        ...
        if listing is None:  # В list_directory ошибка произошла.
            send_response(conn, status=404)  # Без body, просто статуса в заголовках достаточно.
            return
...
def get_request(conn: socket.socket) -> tuple[dict, bool, None] | tuple[dict, bool, str]:
    buffer = b""
    ...
    # Код ниже мржно выделить в отдельную функцию. Но в этом масштабе наверно не нужно.
...
def handle_connection(conn: socket.socket, addr: tuple) -> None:
    try:
        ...
        while keep_alive:
            ...
            if not readable: # Если прошло WAIT_FOR_REQUEST и нет запроса, закрываем соединение.
