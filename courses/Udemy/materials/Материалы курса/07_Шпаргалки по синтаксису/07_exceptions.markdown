...
└── Exception
    ...
    │
    └── KeyboardInterrupt (дублируется как под BaseException)
```
...
| **ExceptionGroup / BaseExceptionGroup (Python 3.11+)** | Позволяет группировать несколько исключений  и обрабатывать их вместе (пока рано, это в асинхронном выполнении нужно).                 |
